...
Создаёт экземпляр системы по индексу меню с параметрами по умолчанию.
`idx` соответствует порядку в `SYSTEMS`: 1=Лоренц, 2=Лотка-Вольтерра,
3=Ван дер Поль, 4=Росслер, 5=двойной маятник.
"""
function make_system(idx::Int)
    if idx == 1; return Models.LorenzSystem(10.0, 28.0, 2.666)
    elseif idx == 2; return Models.LotkaVolterraSystem(1.0, 0.1, 1.5, 0.075)
    elseif idx == 3; return Models.VanDerPolSystem(1.0)
    elseif idx == 4; return Models.RosslerSystem(0.2, 0.2, 5.7)
    else; return Models.DoublePendulumSystem(1.0, 1.0, 1.0, 1.0)
    end
end
...
function run_app()
    ...
    param1_obs       = Observable(10.0)
    param2_obs       = Observable(28.0)
    param3_obs       = Observable(2.666)
    ...
    u_results_obs = lift(
        ...
        sys = if idx == 1; Models.LorenzSystem(p1, p2, p3)
              elseif idx == 2; Models.LotkaVolterraSystem(p1, p2, p3, 0.075)
              elseif idx == 3; Models.VanDerPolSystem(p1)
              elseif idx == 4; Models.RosslerSystem(p1, p2, p3)
              else; Models.DoublePendulumSystem(p1, p2, 1.0, 1.0)
              end
    ...
    on(system_menu.selection) do sel
        ...
        if idx == 1
            set_close_to!(sg_params.sliders[1], 10.0)
            set_close_to!(sg_params.sliders[2], 28.0)
            set_close_to!(sg_params.sliders[3], 2.666)
        elseif idx == 2
            set_close_to!(sg_params.sliders[1], 1.0)
            set_close_to!(sg_params.sliders[2], 0.1)
            set_close_to!(sg_params.sliders[3], 1.5)
        elseif idx == 3
            set_close_to!(sg_params.sliders[1], 1.0)
            set_close_to!(sg_params.sliders[2], 1.0)
            set_close_to!(sg_params.sliders[3], 1.0)
        elseif idx == 4
            set_close_to!(sg_params.sliders[1], 0.2)
            set_close_to!(sg_params.sliders[2], 0.2)
            set_close_to!(sg_params.sliders[3], 5.7)
        else
            set_close_to!(sg_params.sliders[1], 1.0)
            set_close_to!(sg_params.sliders[2], 1.0)
            set_close_to!(sg_params.sliders[3], 1.0)
    ...
    Label(right_panel[r, 1:2], "Параметры системы:",
          color = :gray80, halign = :left, tellwidth = false)
    r += 1
    sg_params = SliderGrid(
        right_panel[r, 1:2],
        (label = "Параметр 1", range = 0.01:0.01:30.0, startvalue = 10.0),
        (label = "Параметр 2", range = 0.01:0.1:50.0,  startvalue = 28.0),
        (label = "Параметр 3", range = 0.01:0.01:10.0, startvalue = 2.666),
        tellwidth = true
    )
    on(sg_params.sliders[1].value) do v; param1_obs[] = v; end
    on(sg_params.sliders[2].value) do v; param2_obs[] = v; end
    on(sg_params.sliders[3].value) do v; param3_obs[] = v; end
